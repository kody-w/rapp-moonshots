...
| “The fixture changes mid-run and fabricates the flip.” | Critical | Fixture bytes and SHA-256 are captured once before baseline. Every trial executes a hash-checked copy of that immutable snapshot. Source hash drift before receipt release raises `FixtureDriftError`; a regression mutates the source after baseline, observes unmixed snapshot outcomes, and confirms no receipt. |
...
| “A flaky fixture can manufacture a flip.” | High | The gate is directional, not merely “different”: baseline must be 3/3 FAIL, every rejected control 3/3 FAIL, and the declared cause 3/3 PASS. A unanimous PASS baseline, PASS→FAIL inversion, mixed baseline/control/cause, or unexpected control pass aborts. Only unanimous FAIL controls increment `earlier_controls_rejected`. |
| “A shortened run passes because its smaller denominators match.” | High | Measurement gates compare exact declared totals: 3 scenarios, 9 baseline failures, 18 control failures, 9 causal passes, 36 trials/cleanups, 3 source verifications, zero residue, and 100% cause accuracy. Every mismatch produces an explicit `failure_reasons` entry; a one-repeat regression must fail. |
