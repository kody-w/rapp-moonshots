...
| “Secrets leak through environment capture.” | Critical | Child environments replace rather than extend `os.environ`. Receipts contain only three allowlisted values, state `inherited_environment_keys: 0`, and never include absolute workspace paths. |
| “Temporary trials pollute the machine.” | Medium | Workspaces are under this track's ignored `.runtime/`, uniquely named, and removed in `finally`. The measured 36/36 trials left zero workspace residue. |
...
| “Concurrent runs cross-contaminate.” | Medium | Every trial has a UUID workspace, receives a fresh environment dictionary, and is cleaned independently. Active runs are capped at two; the registry is lock-protected. |
