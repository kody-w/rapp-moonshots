...
| Distinct model IDs sanitize to the same choice | Suggestion IDs are lowercased and sanitized first; duplicate normalized IDs, including prefix collisions, reject the entire response before state/UI exposure | Rejection can force demo failover |
...
| Model option content leaks through events or What changed | Model IDs, labels, details, prompts, and branches never enter public event semantics; history records application-owned categories, What changed normalizes them, and export recursively drops private detail keys | Application-owned scenario/task semantics remain visible |
...
| Microphone regrant starts recognition under active narration | Explicit recovery invalidates the utterance epoch and cancels synthesis both before permission and immediately before recognition start; stale callbacks cannot schedule another restart | Browser hardware prompts can outlive the page |
| Sensor-free global narration is captured by a newly enabled microphone | Controller creation and microphone enable cancel global synthesis even when controller speaking state is false; acquisition cancels immediately before `getUserMedia` and recognition cancels again immediately before start | Already-rendered audio may take browser/OS time to stop |
...
| Canceling pending AI leaves zero selectable choices | The shared cancellation primitive clears pending state, advances request identity, recomputes options, and reapplies automatic mode before cancel/stop/background rendering | A future option generator could still return an invalid empty state |
| Undo restores an orphaned pending request | A newer input cancels the previous request before snapshot; undo always forces pending false, retains the monotonic request ID, rejects stale delivery, and rebuilds choices | Upstream processing may have completed before local abort |
| 400% zoom triggers clipped landscape columns | Compact landscape defaults to one scroll-safe column; a 620 px app-content container threshold enables columns only when their measured minimum fits | Extreme browser text overrides may increase vertical scrolling |
