...
| Camera freezes after arming | Decoded-frame identity/`currentTime` and derived frame content must advance; unchanged pixels revoke focus and bounded sensor loss requires center recovery | Slow real movement can conservatively pause gaze |
| Covered/static camera reports advancing frames | Mean luminance, variance, and near-identical-content floors block estimator updates, revoke sensor arms, and time out | Thresholds need live-study tuning across cameras and skin tones |
| Confirmation races the watchdog | `confirm()` synchronously requires fresh raw frames, valid changing content, and a fresh processed gaze sample for sensor-derived arms | Manual/switch arms intentionally do not depend on camera freshness |
...
| Old FaceDetector promise resolves after loss/recovery | Each request captures a content-validity epoch; invalidity, loss, recovery reset, fallback, and stop advance it, so the deferred result is dropped before `onSample` | FaceDetector remains an optional coarse estimator, not an authorization source |
